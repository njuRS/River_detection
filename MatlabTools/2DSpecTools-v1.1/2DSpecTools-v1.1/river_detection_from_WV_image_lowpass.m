...
% 1. LOAD DATA %
[Z, dim] = ReadArcGrid('orthoWV01_20150718160144_for_WQ7_207_norm_denoised.asc');
...
figure(s1d)
subplot(2,1,2)
semilogx(fv,Make2DFilt(fv, f, 'lowpass'),'b')
set(gca,'box','off','tickdir','out','ylim',[0 1.01])
xlabel('Radial frequency (m^{-1})')
ylabel('Filter')
% Filter the DEM
Zhp = SpecFilt2D(Z,dx,dy,f,'lowpass');
% Low-pass filter
Zlp = Z - Zhp;
%Zlp = SpecFilt2D(Z,dx,dy,[floLP fhiLP],'lowpass');
...
WriteArcGrid(xhp,yhp,Zhp,'orthowv01_20150718160144_for_wq7_207_lowpass')
WriteArcGrid(xhp,yhp,Zlp,'orthowv01_20150718160144_for_wq7_207_lowpass_left')
